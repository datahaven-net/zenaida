...
def domain_restore(domain_object,
                   res_reason='', pre_data='', post_data='',
                   del_time=None, res_time=None,
                   statement1='', statement2='', other='', ):
#     
#     pending_delete_date = re.search('\d\d\d\d-\d\d-\d\d \d\d:\d\d .\d\d\d\d', st['#text'])
#     if pending_delete_date:
#         t = pending_delete_date.group(0)
#         pending_delete_date = (
#             datetime.datetime.strptime(t[:-6], '%Y-%m-%d %H:%M') + 
#             datetime.timedelta(hours=int(t[-5:-2]))
#         ).strftime('%Y-%m-%dT%H:%M:%S.%fZ')
#     
#     #--- UPDATE DOMAIN
#     if pre_data or post_data or res_reason or del_time or res_time:
#         rgp_restore_report={
#             "pre_data": pre_data or "Pre-delete registration data not provided",
#             "post_data": post_data or "Post-restore registration data not provided",
#             "del_time": del_time or epp_domain_info.get('pending_delete_date', ''),
#             "res_time": res_time or datetime.datetime.strftime(datetime.datetime.utcnow(), '%Y-%m-%dT%H:%M:%SZ'),
#             "res_reason": res_reason or "Customer wish to restore ownership of the domain.",
#             "statement1": statement1 or "The information in this report is correct",
#             "statement2": statement2 or "Generated by .AI site automation process",
#             "other": other or "No other information provided",
#         }
#         update = epp_client.cmd_domain_update(domain, rgp_restore_report=rgp_restore_report)
#     else:
#         update = epp_client.cmd_domain_update(domain, rgp_restore=True)
#     if update['epp']['response']['result']['@code'] != '1000':
#         raise epp_client.EPPCommandFailed(message='EPP domain_update failed with error code: %s' % (
