...
def domain_restore(domain_object, raise_errors=False, log_events=True, log_transitions=True, **kwargs):
    ...
    pending_delete_status = domain_object.epp_statuses.get('pendingDelete')
    ...
    if kwargs.get('pre_data') or kwargs.get('post_data') or kwargs.get('res_reason') or kwargs.get('del_time') or kwargs.get('res_time'):
        rgp_restore_report={
            ...
            "res_reason": kwargs.get('res_reason', 'Customer wish to restore ownership of the domain.'),
        ...
        response = zclient.cmd_domain_update(domain_object.name, rgp_restore_report=rgp_restore_report)
    else:
        response = zclient.cmd_domain_update(domain_object.name, rgp_restore=True)
    if response['epp']['response']['result']['@code'] != '1000':
        raise zerrors.EPPCommandFailed(message='EPP domain_update failed with error code: %s' % (
            response['epp']['response']['result']['@code'], ))
    return True
